...
PtBis <- function(ctt.data, discr.data) {
  ...
  discr.data %>%
    ...
    dplyr::mutate(., crit.one = ifelse(`0` < 0, 0, 1),
                  ...
                  crit.three = ifelse(`2` <= 0.01, 1, ifelse(`2` > 0.01 && `2` < 0.2, 2, 0))) %>%
    dplyr::select(., item, discr, `2`, dplyr::matches("^crit")) -> table3.output
...
Infit<- function(tam.fit) {
  ...
  tam.fit %>%
    ...
    dplyr::select(., fit.output.parameter, fit.output.Infit,fit.output.Infit_t, w.mnsq) %>%
